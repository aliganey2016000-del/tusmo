...
export default async function AdminDashboard() {
  // 1. PARALLEL FETCHING: Dhammaan xogta hal mar ayay wada soo load-garaysaa (Ka dhakhso badan 3x)
  const [
    teacherCount, 
    classCount, 
    pendingApps, 
    ...
    db.student.count({ where: { status: "Active", grade: { in: ["1", "2", "3", "4", "Grade 1", "Grade 2", "Grade 3", "Grade 4"] } } }),
    db.student.count({ where: { status: "Active", grade: { in: ["5", "6", "7", "8", "Grade 5", "Grade 6", "Grade 7", "Grade 8"] } } }),
    db.student.count({ where: { status: "Active", grade: { in: ["9", "10", "11", "12", "Grade 9", "Grade 10", "Grade 11", "Grade 12"] } } }),
  ]);
  ...
  return (
    <div className="space-y-8 text-left">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm">
        ...
        <div className="flex items-center gap-4 bg-slate-50 p-2 rounded-2xl border border-slate-100">
           <div className="flex -space-x-3 ml-2 uppercase">
              {['AD', 'US', 'MN'].map((initials, i) => (
                <div key={i} className="w-8 h-8 rounded-full border-2 border-white bg-slate-200 flex items-center justify-center text-[10px] font-black tracking-tighter">
                  {initials}
                </div>
              ))}
           </div>
          ...
          <div key={i} className={`bg-white p-6 rounded-[2.5rem] border border-slate-100 shadow-sm hover:${stat.shadow} hover:shadow-2xl transition-all duration-500 group relative overflow-hidden text-left`}>
            <div className="relative z-10 flex flex-col gap-4">
               ...
               <div className="flex justify-between items-end">
                  ...
                  <span className={`text-[10px] font-black px-2 py-1 rounded-lg ${i === 3 && stat.value > 0 ? 'bg-rose-50 text-rose-600 animate-bounce' : 'bg-emerald-50 text-emerald-600'}`}>
                    {stat.percentage}
                  </span>
               </div>
            </div>
      ...
      {/* 3. CENTER CONTENT: SCHOOL STATUS & QUICK ACTIONS */}
      <div className="grid lg:grid-cols-3 gap-8">
        
        {/* Left: School Levels Distribution */}
        <div className="lg:col-span-2">
           <div className="bg-white rounded-[2.5rem] p-10 border border-slate-100 shadow-sm relative overflow-hidden h-full">
              ...
              <div className="grid md:grid-cols-2 gap-12">
                 <div className="space-y-8">
                    {/* Progress Bars */}
                    {[
                      { label: "Hoose (1-4)", value: hooseActive, color: "bg-blue-600" },
                      { label: "Dhaxe (5-8)", value: dhaxeActive, color: "bg-emerald-600" },
                      { label: "Sare (9-12)", value: sareActive, color: "bg-orange-500" }
                    ].map((level, idx) => (
                      <div key={idx} className="space-y-3">
                         <div className="flex justify-between text-[10px] font-black uppercase tracking-widest text-slate-500">
                            <span className="flex items-center gap-2">{level.label}</span>
                            <span>{level.value} Arday</span>
                         </div>
                         <div className="w-full h-4 bg-slate-100 rounded-full overflow-hidden">
                            <div className={`h-full ${level.color} rounded-full transition-all duration-1000`} style={{ width: `${(level.value / totalForBar) * 100}%` }} />
                         </div>
                      </div>
                    ))}
                 </div>
        ...
        <div className="space-y-6">
          <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm">
            ...
            <div className="space-y-3">
               <Link href="/dashboard/admin/students" className="group flex items-center gap-4 bg-slate-50 hover:bg-blue-600 p-4 rounded-2xl transition-all duration-300">
                  <div className="bg-white p-2 rounded-xl group-hover:bg-blue-500 transition-colors shadow-sm">
               ...
               <Link href="/dashboard/admin/inbox" className="group flex items-center gap-4 bg-slate-50 hover:bg-emerald-600 p-4 rounded-2xl transition-all duration-300 relative">
                  <div className="bg-white p-2 rounded-xl group-hover:bg-emerald-500 transition-colors shadow-sm">
                  ...
                  {unreadMessages > 0 && <span className="absolute right-4 bg-rose-500 text-white text-[10px] font-black px-2 py-0.5 rounded-full animate-bounce">{unreadMessages}</span>}
               </Link>
            </div>
          </div>
